...
function Post({post}){
    ...
    useEffect(() => {
        const fetchUser = async () => {
            const res = await axios.get(`https://bee-social-app.herokuapp.com/api/users?userId=${post.userId}`);
            setUser(res.data);
        }
        fetchUser();
    },[post.userId]);
    ...
    const likeHandler = () => {
        try {
            axios.put("https://bee-social-app.herokuapp.com/api/posts/"+post._id+"/like", {userId : currentUser._id});
        } catch (error) {
            console.log(error);
        }
