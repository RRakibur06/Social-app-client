...
export default function Conversation({conversation, currentUser, message}) {
    ...
    useEffect(()=>{
        ...
        const getUser = async () => {
            try {
                const res = await axios.get("https://bee-social-app.herokuapp.com/api/users?userId=" + friendId);
                setUser(res.data);
            } catch (error) {
                console.log(error);
            }
    ...
    useEffect(()=>{
        ...
        const getMessages = async () => {
            try {
                const res = await axios.get("https://bee-social-app.herokuapp.com/api/messages/" + conversation._id);
