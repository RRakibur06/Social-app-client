...
function Profile(){
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            const res = await axios.get(`https://bee-social-app.herokuapp.com/api/posts/profile/${username}`);
            setPosts(
                res.data.sort((p1, p2)=>{
    ...
    useEffect(() => {
        const fetchUser = async () => {
            const res = await axios.get(`https://bee-social-app.herokuapp.com/api/users?username=${username}`);
            setUser(res.data);
        }
        fetchUser();
    },[username]);
    ...
    useEffect(() => {
        const getFriends = async () => {
            try {
                const res = await axios.get("https://bee-social-app.herokuapp.com/api/users/friends/"+user._id);
                setFriends(res.data);    
            } catch (error) {
                console.log(error);
            }
    ...
    const handleClick = async () => {
        try {
            if(followed){
                await axios.put("https://bee-social-app.herokuapp.com/api/users/"+user._id+"/unfollow", {userId : currentUser._id});
                dispatch({type : "UNFOLLOW", payload : user._id});
            }
            else{
                await axios.put("https://bee-social-app.herokuapp.com/api/users/"+user._id+"/follow", {userId : currentUser._id});
